...
fig.savefig(fig_dir / fig3_name.replace('.png', '.pdf'))
...
gs = gridspec.GridSpec(5, 2, height_ratios=(3, 1, 2, 4, 3), hspace=0.07, wspace=0.07, left=0.18, right=0.99, top=0.84, bottom=0.25)
...
plot_group_ranking_scatter(df_rd, col_uhat='uhat', CI=CI, legend=True, highlight_top=highlight_top,
                           fontsize=fontsize, ax=ax00, hue='Superclass (top 15)', hue_order=hue_order, palette=palette)
plot_group_ranking_scatter(df_nc, col_uhat='uhat', CI=CI, legend=False, highlight_top=highlight_top,
...
plot_group_counts(df_rd, fontsize=fontsize, ax=ax20, hue='Superclass (top 15)', hue_order=hue_order, palette=palette, highlight_top=highlight_top)
plot_group_counts(df_nc, fontsize=fontsize, ax=ax21, hue='Superclass (top 15)', hue_order=hue_order, palette=palette, highlight_top=highlight_top)

# C: Distribution of PODs as violins and boxplots (highest ranked classes)
ax30 = fig.add_subplot(gs[3, 0], sharex=ax20)
ax31 = fig.add_subplot(gs[3, 1], sharex=ax21, sharey=ax30)
plot_group_ranking_violins(df_rd, fontsize=fontsize, ax=ax30, invert_y=True, highlight_top=highlight_top,
                           hue='Superclass (top 15)', hue_order=hue_order, palette=palette)
plot_group_ranking_violins(df_nc, fontsize=fontsize, ax=ax31, highlight_top=highlight_top,
                           hue='Superclass (top 15)', hue_order=hue_order, palette=palette)

# D: Distribution of uncertainty as violins and boxplots (highest ranked classes)
ax40 = fig.add_subplot(gs[4, 0], sharex=ax20)
ax41 = fig.add_subplot(gs[4, 1], sharex=ax21, sharey=ax40)
plot_group_ranking_violins(df_rd, y_col='uhat', y_label='uncertainty ({CI}% CI width)'.format(CI=CI), highlight_top=highlight_top,
                           fontsize=fontsize, ax=ax40, hue='Superclass (top 15)', hue_order=hue_order, palette=palette)
plot_group_ranking_violins(df_nc, y_col='uhat', y_label='uncertainty ({CI}% CI width)'.format(CI=CI), highlight_top=highlight_top,
...
fig.savefig(fig_dir / fig4_name, dpi=600, transparent=True)
...
for n, ax in enumerate([ax00, ax01, ax10, ax11, ax20, ax21]):
    ax.text(-0.15, 1, letters[n], transform=ax.transAxes, fontsize=fontsize + 1, fontweight='bold', va='top',ha='right')
